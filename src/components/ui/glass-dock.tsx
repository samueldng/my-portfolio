...
export const GlassDock = React.forwardRef<HTMLDivElement, GlassDockProps>(
    ({ items, className, dockClassName, ...props }, ref) => {
        ...
        const [isDark, setIsDark] = useState(true); // Default to dark since portfolio is dark

        // Kept isDark logic just in case, but portfolio is default dark
        React.useEffect(() => {
            const checkTheme = () => {
                setIsDark(document.documentElement.classList.contains('dark') || document.body.classList.contains('dark-mode') || document.body.classList.contains('interactive-mode'));
            };
            checkTheme();
            ...
            <div ref={ref} className={cn('w-max', className)} {...props} >
                <div
                    className={cn(
                        ...
                        "glass-border bg-gray-950/80", // the user's theme bg
                    ...
                    <AnimatePresence>
                        {hoveredIndex !== null && (
                            <motion.div
                                ...
                                animate={{ opacity: 1, scale: 1, y: -60, x: getTooltipPosition(hoveredIndex), }}
                            ...
                            >
                                <div
                                    className={cn(
                                        ...
                                        'bg-white text-black', // Reversed for dark theme context
                    ...
                    {items.map((el, index) => {
                        ...
                        const handleClick = () => {
                            if (el.onClick) {
                                ...
                                window.open(el.href, '_blank'); // Open links in new tab
                            }
                        };
                        return (
                            <div
                            ...
                            >
                                <motion.div
                                    whileTap={{ scale: 0.95 }}
                                    animate={{ scale: isHovered ? 1.1 : 1, y: isHovered ? -3 : 0, }}
